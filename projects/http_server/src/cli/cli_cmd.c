...
//AT+CWSAP_CUR="ssv6266","1234567890",5,3 
char __hscmd_cwsap(char * ssid,char *password ,char* chl,char* ecn)
{
	uint32_t tmp_ip0=0;
	uint32_t tmp_ip1=0;
	uint32_t tmp_ip2=0;
	...
	uint8_t *ap_ssid= ssid;
	uint8_t *ap_pwd = password;
	uint8_t *ap_chl = chl;
	uint8_t *ap_ecn = ecn;
	uint8_t *ap_max_conn = NULL;
	uint8_t *ap_hidden = NULL;
	...
	isoftap_custom_config.start_ip = 0xC0A80402;//192.168.4.2
	isoftap_custom_config.end_ip   = 0xC0A80405;//192.168.4.5
	isoftap_custom_config.gw	   = 0xC0A80401;//192.168.4.1
	isoftap_custom_config.subnet   = 0xFFFFFF00;//255.255.255.0		
	isoftap_custom_config.max_sta_num = 4;

	if(ap_max_conn != NULL)
	{
		isoftap_custom_config.max_sta_num = strtoul(ap_max_conn, NULL, 10);
	...
	isoftap_custom_config.encryt_mode = strtoul(ap_ecn, NULL, 10);
	isoftap_custom_config.channel = strtoul(ap_chl, NULL, 10);
	
	isoftap_custom_config.keylen = strlen(ap_pwd);
	memset(isoftap_custom_config.key,0,sizeof(isoftap_custom_config.key));	 
	memcpy(isoftap_custom_config.key,ap_pwd,isoftap_custom_config.keylen);
	
	isoftap_custom_config.ssid_length = strlen(ap_ssid);
	memset(isoftap_custom_config.ssid,0,sizeof(isoftap_custom_config.ssid));   
	memcpy(isoftap_custom_config.ssid,ap_ssid,isoftap_custom_config.ssid_length);
	...
	int ret = softap_set_custom_conf(&isoftap_custom_config);
	if(ret != 0)
	{
		goto __err;
	}
...
int At_httpd_start (s32 argc, char *argv[])
{   
/*	DUT_wifi_start(DUT_STA);
	OS_MsDelay(1000);
	if(__hscmd_cwsap("ssv_web_cfg","1234567890","5","2") == 0)
	{
		httpd_init();
		scan_AP(scan_cbfunc);
	}
	...
	if(__hscmd_cwsap("ssv_web_cfg","1234567890","5","2") == 0)
	{
		creat_http_task(8080,4);
	}
	return ERROR_SUCCESS;

}

int At_httpd_stop (s32 argc, char *argv[])
{
	//httpd_close();
	//
	delete_http_task();
...
{
    { "httpserver_start"  ,     At_httpd_start,     "httpd start"},
    { "httpserver_stop",         At_httpd_stop,      "httpd stop"},
